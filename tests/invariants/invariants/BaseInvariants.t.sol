...
abstract contract BaseInvariants is HandlerAggregator {
    ...
    function assert_BASE_INVARIANT_C(address asset) internal {
        ...
        uint256 debtTokenTotalSupply = IERC20(protocolTokens[asset].variableDebtTokenAddress).totalSupply();
        uint256 liabilityFreeAmount =
            (aTokenTotalSupply > debtTokenTotalSupply) ? aTokenTotalSupply - debtTokenTotalSupply : 0;
        liabilityFreeAmount =
            liabilityFreeAmount > NUMBER_OF_ACTORS ? liabilityFreeAmount - NUMBER_OF_ACTORS : liabilityFreeAmount;
    ...
    function assert_BASE_INVARIANT_F(address asset) internal {
        ...
        assertApproxEqAbs(virtualBalance + currentDebt, _getRealTotalSupply(asset), 10, BASE_INVARIANT_F);
    }
    ...
    function assert_BORROWING_INVARIANT_B(address asset) internal {
        ...
        for (uint256 i; i < NUMBER_OF_ACTORS; i++) {
            if (IERC20(protocolToken.variableDebtTokenAddress).balanceOf(actorAddresses[i]) == 0) {} else {
                assertTrue(
