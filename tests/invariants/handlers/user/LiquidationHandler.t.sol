...
contract LiquidationHandler is BaseHandler {
    ...
    function liquidationCall(uint256 debtToCover, bool receiveAToken, uint8 i, uint8 j, uint8 k, uint8 l)
        external
        setup
    {
        bool success;
        ...
        helper_violatorCollateralReserveValueBefore = _getUserReserveValueInBaseCurrency(
            targetAsset, IERC20(protocolTokens[targetAsset].aTokenAddress).balanceOf(user)
        );

        helper_violatorDebtBalanceBefore =
            IERC20(protocolTokens[_getRandomBaseAsset(k)].variableDebtTokenAddress).balanceOf(user);

        helper_violatorDebtReserveValueBefore =
            _getUserReserveValueInBaseCurrency(_getRandomBaseAsset(k), helper_violatorDebtBalanceBefore);
        ...
        if (success) {
            ...
            uint256 amountWithCloseFactor = violatorDebtReserveValueAfter.percentMul(DEFAULT_LIQUIDATION_CLOSE_FACTOR);
...

            {
                uint256 userCollateralBalance =
                    IERC20(protocolTokens[_getRandomBaseAsset(j)].aTokenAddress).balanceOf(user);
            ...
            uint256 deficitDelta = pool.getReserveDeficit(_getRandomBaseAsset(k)) - helper_debtAssetDeficitBefore;
