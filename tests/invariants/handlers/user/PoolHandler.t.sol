...
contract PoolHandler is BaseHandler {
    ...
    function mintToTreasury(uint8 i) external setup {
        ...
        _before();
        (success, returnData) = actor.proxy(target, abi.encodeWithSelector(IPool.mintToTreasury.selector, assets));
    ...
    function setUserEMode(uint8 i) external setup {
        ...
        _before();
        (success, returnData) = actor.proxy(target, abi.encodeWithSelector(IPool.setUserEMode.selector, eModeCategory));
    ...
    function eliminateReserveDeficit(uint256 amount, uint8 i) external {
        ...
        assertApproxEqAbs(defaultVarsBefore.totalSupply - amount, defaultVarsAfter.totalSupply, 1, DM_HSPOST_B);
