...
contract LendingHandler is BaseHandler {
    ...
    function supply(uint256 amount, uint8 i, uint8 j) external setup {
        ...
        uint256 onBehalfOfATokenBalanceBefore = IERC20(protocolTokens[asset].aTokenAddress).balanceOf(onBehalfOf);

        _before();
        (success, returnData) =
            actor.proxy(target, abi.encodeWithSelector(IPool.supply.selector, asset, amount, onBehalfOf, 0));
        ...
        if (success) {
            ...
            assertEq(IERC20(asset).balanceOf(address(actor)), senderUnderlyingBalanceBefore - amount, LENDING_HPOST_D);
    ...
    function withdraw(uint256 amount, uint8 i, uint8 j) external setup {
        ...
        uint256 actorATokenBalanceBefore = IERC20(protocolTokens[asset].aTokenAddress).balanceOf(address(actor));
