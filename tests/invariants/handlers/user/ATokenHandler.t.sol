...
contract ATokenHandler is BaseHandler {
    ...
    function transfer(uint256 amount, uint8 i, uint8 j) external setup {
        ...
        _before();
        (success, returnData) = actor.proxy(target, abi.encodeWithSelector(IERC20.transfer.selector, recipient, amount));
    ...
    function transferFrom(uint256 amount, uint8 i, uint8 j, uint256 u) external setup {
        ...
        _before();
        (success, returnData) =
            actor.proxy(target, abi.encodeWithSelector(IERC20.transferFrom.selector, sender, recipient, amount));
    ...
    function increaseAllowance(uint256 addedValue, uint8 i, uint8 j) external setup {
        ...
        (success, returnData) = actor.proxy(
            target, abi.encodeWithSelector(IncentivizedERC20.increaseAllowance.selector, spender, addedValue)
        );
    ...
    function decreaseAllowance(uint256 subtractedValue, uint8 i, uint8 j) external setup {
        ...
        (success, returnData) = actor.proxy(
            target, abi.encodeWithSelector(IncentivizedERC20.decreaseAllowance.selector, spender, subtractedValue)
        );
