...
contract RevenueSplitterTest is StdUtils, Test {
    ...
    function test_splitFunds_fuzz_max(uint256 amountA, uint256 amountB) public {
        vm.assume(
            amountA <= (type(uint256).max - HALF_PERCENTAGE_FACTOR) / revenueSplitter.SPLIT_PERCENTAGE_RECIPIENT_A()
        );
        vm.assume(
            amountB <= (type(uint256).max - HALF_PERCENTAGE_FACTOR) / revenueSplitter.SPLIT_PERCENTAGE_RECIPIENT_A()
        );
    ...
    function _splitNativeFunds_action(uint256 amountA) internal {
        ...
        assertEq(address(revenueSplitter).balance, amountA, "Splitter balance should be amount received by fallback");
    ...
    function test_splitNativeFund_walletNotAcceptingFunds() public {
        ...
        assertEq(address(revenueSplitterInstance).balance, amountA, "Splitter balance should be equal to amountA");
        assertEq(recipientA.balance, 0, "ETH balance of recipient A");
        ...
        assertEq(recipientA.balance, recipientABalanceA, "ETH balance of recipient A");
        assertEq(
            recipientC.balance, 0, "ETH balance of recipient C should be zero due it does not contain fallback function"
        );
