...
contract BatchTestProcedures is Test, DeployUtils, FfiUtils, DefaultMarketInput {
        ...
        )
    {
        (setupContract, initialReport) = AaveV3BatchOrchestration._deploySetupContract(
            roles.marketOwner, roles, config, deployedContracts
        );
        ...
        poolReport = AaveV3BatchOrchestration._deployPoolImplementations(initialReport.poolAddressesProvider, flags);

        peripheryReport = AaveV3BatchOrchestration._deployPeripherals(
            roles, config, initialReport.poolAddressesProvider, address(setupContract)
        );
    ...
    {
        (
            initialReport, gettersReport1, poolReport, peripheryReport, miscReport, setupContract
        ...
        paraswapReport =
            AaveV3BatchOrchestration._deployParaswapAdapters(roles, config, initialReport.poolAddressesProvider);

        gettersReport2 = AaveV3BatchOrchestration._deployGettersBatch2(
            setupReport.poolProxy, roles.poolAdmin, config.wrappedNativeToken, flags.l2
        );
        ...
        returns (ConfiguratorInputTypes.InitReserveInput[] memory)
    {
        ...
        for (uint256 x; x < listings; ++x) {
            TestnetERC20 listingToken =
                new TestnetERC20(_concatStr("Token", x), _concatStr("T", x), uint8(10 + x), poolAdminUser);
            TestVars memory t;
