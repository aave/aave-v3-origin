...
contract ProtocolV3TestBase is DiffUtils {
    ...
    function _writeReserveConfigs(string memory path, ReserveConfig[] memory configs, IPool pool) internal virtual {
        ...
        for (uint256 i = 0; i < configs.length; i++) {
            ...
            vm.serializeString(key, "variableDebtTokenSymbol", IERC20Detailed(config.variableDebtToken).symbol());
            vm.serializeString(key, "variableDebtTokenName", IERC20Detailed(config.variableDebtToken).name());
        ...
        pure
    {
        ...
        require(config.liquidationBonus == expectedConfig.liquidationBonus, "_validateReserveConfig: INVALID_LIQ_BONUS");
        require(
        ...
        require(config.reserveFactor == expectedConfig.reserveFactor, "_validateReserveConfig: INVALID_RESERVE_FACTOR");
        ...
        require(config.supplyCap == expectedConfig.supplyCap, "_validateReserveConfig: INVALID_SUPPLY_CAP");
        require(config.borrowCap == expectedConfig.borrowCap, "_validateReserveConfig: INVALID_BORROW_CAP");
        require(config.debtCeiling == expectedConfig.debtCeiling, "_validateReserveConfig: INVALID_DEBT_CEILING");
        require(
    ...
    function _validateEmodeCategory(
        ...
        DataTypes.CollateralConfig memory cfg = IPool(poolAddress).getEModeCategoryCollateralConfig(uint8(category));
        require(
