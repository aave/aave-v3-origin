...
} from "../../../src/contracts/protocol/configuration/PoolAddressesProvider.sol";
import {PoolInstance} from "../../../src/contracts/instances/PoolInstance.sol";
import {
    MockInitializableV1,
    MockInitializableV2
} from "../../../src/contracts/mocks/upgradeability/MockInitializableImplementation.sol";
...
contract PoolAddressesProviderTests is TestnetProcedures {
    ...
    function test_setPoolImpl() public returns (PoolAddressesProvider, address) {
        ...
        address poolImplementation = address(new PoolInstance(IPoolAddressesProvider(address(provider))));
    ...
    function test_setPoolImpl_upgrade() public {
        ...
        address poolImplementation = address(new MockPoolInherited(IPoolAddressesProvider(address(provider))));
    ...
    function test_setPoolConfiguratorImpl_upgrade() public {
        (PoolAddressesProvider provider, address currentImplementation) = test_setPoolConfiguratorImpl();
        address implementation = address(new MockPoolInherited(IPoolAddressesProvider(address(provider))));
