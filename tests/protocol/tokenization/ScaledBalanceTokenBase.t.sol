...
contract ScaledBalanceTokenBaseEdgeTests is TestnetProcedures {
    ...
    function test_scaled_balance_token_base_alice_transfer_to_bob_accrues_interests() public {
        ...
        mockScaledToken.setStorage(alice, bob, previousIndex, aliceScaledBalanceBefore, bobScaledBalanceBefore);

        uint256 senderBalanceIncrease =
            aliceScaledBalanceBefore.rayMul(expectedIndex) - aliceScaledBalanceBefore.rayMul(previousIndex);
        uint256 recipientBalanceIncrease =
            bobScaledBalanceBefore.rayMul(expectedIndex) - bobScaledBalanceBefore.rayMul(previousIndex);
