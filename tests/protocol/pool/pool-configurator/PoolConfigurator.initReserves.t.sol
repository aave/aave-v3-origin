    ...
    function test_initReserves_validNumberOfAssets(TestVars[128] memory t, uint8 length) public {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            emit ReserveInitialized(
                input[i].underlyingAsset, address(0), address(0), address(0), input[i].interestRateStrategyAddress
            );
        }
        // Perform action
        vm.prank(poolAdmin);
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            // Perform assertions
            {
                ...
                assertEq(address(AToken(aTokenProxy).getIncentivesController()), initConfig.incentivesController);
                ...
                assertEq(address(AToken(variableDebtProxy).getIncentivesController()), initConfig.incentivesController);
            }
