...
contract PoolLiquidationTests is TestnetProcedures {
    ...
    function test_liquidate_variable_borrow_same_collateral_and_borrow() public {
        ...
        if (params.receiveAToken) {
            (address atoken,,) = contracts.protocolDataProvider.getReserveTokensAddresses(params.collateralAsset);
    ...
    function test_liquidate_variable_borrow_repro() public {
        ...
        (,, address varDebtToken) = contracts.protocolDataProvider.getReserveTokensAddresses(params.debtAsset);
    ...
    function test_full_liquidate_multiple_variable_borrows() public {
        ...
        (,, address variableDebtToken) = contracts.protocolDataProvider.getReserveTokensAddresses(params.debtAsset);
        (address atokenAddress,,) = contracts.protocolDataProvider.getReserveTokensAddresses(params.collateralAsset);
    ...
    function test_self_liquidate_isolated_position_shoulDisableCollateral() public {
        ...
        emit IsolationModeTotalDebtUpdated(
            params.collateralAsset, ((borrowAmount - params.actualDebtToLiquidate) / 1e4)
        );
    ...
    function test_self_liquidate_isolated_position_shoulEnableCollateralIfIsolatedSupplier() public {
        uint256 borrowAmount = 11000e6;
        vm.startPrank(poolAdmin);
        IAccessControl(address(contracts.aclManager)).grantRole(keccak256("ISOLATED_COLLATERAL_SUPPLIER"), alice);
        contracts.poolConfiguratorProxy.setDebtCeiling(tokenList.wbtc, 12_000_00);
        contracts.poolConfiguratorProxy.setBorrowableInIsolation(tokenList.usdx, true);
        vm.stopPrank();
    ...
    function test_liquidate_borrow_bad_debt() public {
        ...
        vm.expectEmit(address(contracts.poolProxy));
        emit LiquidationLogic.DeficitCreated(params.user, tokenList.usdx, userDebtBefore - params.actualDebtToLiquidate);
        vm.prank(bob);
        contracts.poolProxy
            .liquidationCall(
                params.collateralAsset,
    ...
    function _afterLiquidationChecksVariable(
        ...
        (address collateralBalance,,) = contracts.protocolDataProvider.getReserveTokensAddresses(params.collateralAsset);
        if (params.receiveAToken) {
            assertEq(
