...
contract BridgeLogicTests is TestnetProcedures {
    ...
    function _checkMintUnbackedEffects(MintUnbackedParams memory i)
        internal
        returns (uint256 expectedIndex, uint256 balanceIncrease, uint256 expectedUnbacked)
    {
        DataTypes.ReserveDataLegacy memory reserveData = contracts.poolProxy.getReserveData(i.underlyingToken);
        ...
        balanceIncrease = scaledBalance.rayMul(expectedIndex)
        ...
        vm.expectEmit(i.aTokenAddress);
        emit Mint(i.user, i.onBehalfOf, i.unbackedAmount + balanceIncrease, balanceIncrease, expectedIndex);
    ...
    function _checkBackUnbackedEffects(BackUnbackParams memory p)
        internal
        returns (
        ...
        )
    {
        ...
        DataTypes.ReserveDataLegacy memory reserveData = contracts.poolProxy.getReserveData(p.underlyingToken);

        vars.protocolFeeBps = contracts.poolProxy.BRIDGE_PROTOCOL_FEE();
        vars.backingAmount = (p.backedAmount < reserveData.unbacked) ? p.backedAmount : reserveData.unbacked;
        ...
        vars.cumulatedLiquidityInterest =
            MathUtils.calculateLinearInterest(reserveData.currentLiquidityRate, reserveData.lastUpdateTimestamp);
        ...
        uint256 newTotalLiquidity =
            IAToken(p.aTokenAddress).totalSupply() + uint256(reserveData.accruedToTreasury).rayMul(expectedIndex);

        expectedIndex =
            (vars.feeToLP.wadToRay().rayDiv(newTotalLiquidity.wadToRay()) + WadRayMath.RAY).rayMul(expectedIndex);

        expectedAccruedToTreasury = reserveData.accruedToTreasury + vars.feeToProtocol.rayDiv(expectedIndex).toUint128();
    ...
    function _unbackedMintAction() internal {
        ...
        (uint256 expectedIndex, uint256 balanceIncrease, uint256 expectedUnbacked) =
        ...
        assertEq(reserveData.unbacked, expectedUnbacked, "Current unbacked amount not match expected unbacked amount");
    }

    function _backUnbackedAction(uint256 backAmount, uint256 fee, address user, address underlyingToken, address aToken)
        internal
    {
        ...
        (uint256 expectedIndex,, uint256 expectedUnbacked, uint256 expectedAccruedToTreasury) =
