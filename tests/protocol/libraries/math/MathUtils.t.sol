...
contract MathUtilsTests is Test {
    ...
    function test_calculateCompoundInterest_1() public {
        uint40 previousTimestamp = uint40(block.timestamp);
        vm.warp(block.timestamp + (365 days / 2));
        assertEq(w.calculateCompoundedInterest(1.0005e27, previousTimestamp), 1.646239361880034706419516e27);
    }
    ...
    function test_calculateCompoundInterest_2() public {
        ...
        vm.warp(block.timestamp + (365 days / 2));
        assertEq(
            w.calculateCompoundedInterest(1.0005e27, previousTimestamp, block.timestamp), 1.646239361880034706419516e27
        );
    }

    function test_calculateCompoundInterest_edge() public {
        vm.warp(block.timestamp + (365 days / 2));
        assertEq(w.calculateCompoundedInterest(1.0005e27, uint40(block.timestamp), block.timestamp), 1e27);
    }
}
