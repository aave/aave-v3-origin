...
contract LibraryPreCompileTwo is FfiUtils, Script, DeployUtils {
    ...
    function _deployAndWriteLibrariesConfig() internal {
        ...
        IMetadataReporter metadataReporter =
            IMetadataReporter(_deployFromArtifacts("MetadataReporter.sol:MetadataReporter"));
        ...
        string memory sedCommand = string(
            abi.encodePacked(
                "sed -i.bak -r 's (FOUNDRY_LIBRARIES=.*) \\1", librariesSolcString, " ' .env && rm .env.bak"
            )
        );
        ...
        console.log("LibraryPreCompileTwo: FOUNDRY_LIBRARIES updated with all AaveV3 libraries via CREATE2.");
    }
    ...
    function verifyEnvironment() internal {
        ...
        if (found == false) {
            revert("LibraryPreCompileTwo: FOUNDRY_LIBRARIES not found, please run LibraryPrecompileOne first.");
        }
    }
}
