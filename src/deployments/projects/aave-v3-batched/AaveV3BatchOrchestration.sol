...
library AaveV3BatchOrchestration {
    function deployAaveV3(
        ...
        (AaveV3SetupBatch setupBatch, InitialReport memory initialReport) =
        ...
        PoolReport memory poolReport = _deployPoolImplementations(initialReport.poolAddressesProvider, flags);

        PeripheryReport memory peripheryReport =
            _deployPeripherals(roles, config, initialReport.poolAddressesProvider, address(setupBatch));
        ...
        ParaswapReport memory paraswapReport =
            _deployParaswapAdapters(roles, config, initialReport.poolAddressesProvider);

        AaveV3GettersBatchTwo.GettersReportBatchTwo memory gettersReport2 =
            _deployGettersBatch2(setupReport.poolProxy, roles.poolAdmin, config.wrappedNativeToken, flags.l2);
        ...
        ConfigEngineReport memory configEngineReport =
            _deployHelpersBatch1(setupReport, miscReport, peripheryReport, tokensReport);

        StaticATokenReport memory staticATokenReport =
            _deployHelpersBatch2(setupReport.poolProxy, setupReport.rewardsControllerProxy, pa);
    ...
    function _deployGettersBatch2(address poolProxy, address poolAdmin, address wrappedNativeToken, bool l2Flag)
        internal
        returns (AaveV3GettersBatchTwo.GettersReportBatchTwo memory)
    {
        ...
        return AaveV3GettersProcedureTwo.GettersReportBatchTwo({wrappedTokenGateway: address(0), l2Encoder: address(0)});
    }
    ...
    function _deployPoolImplementations(address poolAddressesProvider, DeployFlags memory flags)
        internal
    ...
    function _deployParaswapAdapters(Roles memory roles, MarketConfig memory config, address poolAddressesProvider)
        internal
        returns (ParaswapReport memory)
    {
        if (config.paraswapAugustusRegistry != address(0)) {
            AaveV3ParaswapBatch parawswapBatch = new AaveV3ParaswapBatch(roles.poolAdmin, config, poolAddressesProvider);
            return parawswapBatch.getParaswapReport();
        }

        return ParaswapReport({
    ...
    function _deployTokens(address poolProxy) internal returns (AaveV3TokensBatch.TokensReport memory) {
